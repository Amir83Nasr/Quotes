...
function withMouseTracking<P>(Wrapped: ComponentType<P & { x: number; y: number }>) {
return function Enhanced(props: P) {
const [pos, setPos] = useState({ x: 0, y: 0 })
return (
<div onMouseMove={(e) => setPos({ x: e.clientX, y: e.clientY })} style={{ border: "1px solid", padding: 16 }}>
<p>Mouse: {pos.x}, {pos.y}</p>
<Wrapped {...props} x={pos.x} y={pos.y} />
</div>
)
}
}

function Display({ x, y }: { x: number; y: number }) {
return <p>Display: ({x}, {y})</p>
}

const MouseTracker = withMouseTracking(Display)`}
language="tsx"
explanation="withMouseTracking is a Higher-Order Component: it adds mouse tracking state and passes x/y as props to the wrapped component, layering behavior without modifying it."
preview
/>

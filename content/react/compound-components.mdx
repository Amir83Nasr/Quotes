...
<AccordionContext.Provider value={{ openItem, toggle: (id) => setOpenItem(prev => prev === id ? null : id) }}>
{children}
...
return <button onClick={() => ctx.toggle(id)}>{children}</button>
}
function AccordionPanel({ id, children }: { id: string; children: ReactNode }) {
const ctx = useContext(AccordionContext)!
return ctx.openItem === id ? <div>{children}</div> : null
}
...
// </Accordion>`}
  language="tsx"
  explanation="Compound components share implicit state via Context. The `<Accordion>`provider manages which item is open; children call`toggle()`via`useContext`. This pattern creates a declarative, flexible API."
/>

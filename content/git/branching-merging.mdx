...
<LearningObjectives
  objectives={[
    ...
    "List all branches and see which one is active with git branch",
...
When the feature is done, switch to the branch you want to merge _into_ (usually `main`), then merge:
...
**Merge commit** — if `main` _has_ moved on (someone else committed), Git creates a new commit that ties the two histories together.

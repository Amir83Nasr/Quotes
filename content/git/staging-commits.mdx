...
You now know Git records snapshots. This lesson is about doing it _well_: staging exactly the right changes and writing commit messages that make your history readable months later.
...
<LearningObjectives
  objectives={[
    ...
    "Stage individual hunks with git add -p for surgical, focused commits",
    "Browse commit history visually with git log --oneline --graph",
  ]}
/>
...
A commit does two things: it snapshots your code, and it _explains_ the change. The message is documentation your future teammates (and future you) will read constantly.
...
| Explain _why_ in the body | (nothing)     | "Users hit a 500 when the cart was empty" |

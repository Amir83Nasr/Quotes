...
<AlertDescription>A rebased branch produces a clean, linear history with no "Merge branch 'main'" noise. When your PR merges, the project log reads like a tidy sequence of intentional changes rather than a tangle.</AlertDescription>
</Alert>
...
<AlertDescription>Merge is always safe; rebase is safe only on private history. When you're unsure whether someone else has your commits, choose merge. You can always rebase local work before it's ever shared.</AlertDescription>
</Alert>
...
<Quiz questions={[
  {
    question: "How does rebase differ from merge when combining a feature branch with main?",
    options: [
      "Rebase deletes the feature branch automatically",
      "Rebase replays your commits on top of main as new commits, giving a linear history",
      "Rebase is just a slower merge",
      "Rebase uploads your commits to the remote"
    ],
    correctIndex: 1,
    explanation: "Rebase recreates your commits on top of the target branch (new SHAs), producing a straight-line history with no merge commit. Merge instead ties the two histories together with a merge commit.",
  },
  {
    ...
    options: [
      ...
      "Rebase main into every feature branch daily"
    ],
    correctIndex: 1,
    explanation: "Rebasing rewrites commits into new ones. Doing that to history others already have forces a painful divergence — so only rebase private, unpushed commits.",
  },
  {
    question: "During an interactive rebase, which action collapses a commit into the previous one?",
    options: ["pick", "reword", "squash", "drop"],
    correctIndex: 2,
    explanation: "'squash' merges a commit into the one before it — the standard way to combine several WIP commits into a single clean commit.",
  },
]} />

...
<LessonCodeExample
  ...
  explanation="Verifies the password with bcrypt.compare, signs a JWT, stores it in an httpOnly cookie, and returns the user's id and email."
...
// Verify password — return 401 if mismatch or user not found
const valid = user && (await bcrypt.compare(password ?? "", user.passwordHash))
if (!valid) {
return Response.json({ message: "Invalid credentials" }, { status: 401 })
}

// Sign JWT and store in httpOnly cookie
const token = await signToken({ userId: user.id, email: user.email })
const cookieStore = await cookies()
cookieStore.set("session", token, {
httpOnly: true,
...
sameSite: "lax",
path: "/",
maxAge: 60 * 60 \_ 24 \* 7, // 7 days
})

// Return user info (never expose the password hash)
return Response.json({ id: user.id, email: user.email })
}`}
/>

...
function enrichActiveState(items: NavItem[], currentPath: string): NavItem[] {
  return items.map((item) => {
    ...
    const children = item.children
      ? enrichActiveState(item.children, currentPath)
      : undefined
    const isExpanded =
      isActive ||
...
function SidebarGroup({
  ...
  const expanded = userToggled !== null ? userToggled : item.isExpanded
  const toggle = useCallback(() => {
    setUserToggled((prev) => (prev !== null ? !prev : !item.isExpanded))
  }, [item.isExpanded])
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn(
            "absolute top-0 right-0 z-10 h-full w-1.5 translate-x-1/2 cursor-col-resize transition-colors",
            isResizing ? "bg-primary/40" : "hover:bg-border"
          )}
